...
pipe = WanUniAnimateVideoPipeline.from_model_manager(model_manager, torch_dtype=torch.bfloat16, device=f"cuda:{dist.get_rank()}", 
...
for path_dir_per in test_list_path:
    ...
    if dist.get_rank() == 0:
        ...
        save_video(video_out, "outputs/video_usp_480P_{}_{}.mp4".format(ref_image_path.split('/')[-1], pose_file_path.split('/')[-1]), fps=15, quality=5)
