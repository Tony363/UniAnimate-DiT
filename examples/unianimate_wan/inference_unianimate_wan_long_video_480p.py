...
pipe = WanUniAnimateLongVideoPipeline.from_model_manager(model_manager, torch_dtype=torch.bfloat16, device="cuda")
...
for path_dir_per in test_list_path:
    ...
    video = pipe(
        ...
        width=width,
        context_size = 21,
        context_overlap = 4, # larger than 2, (context_overlap-2)*4 frames are overlapped
    ...
    save_video(video_out, "outputs/video_480P_long_{}_{}.mp4".format(ref_image_path.split('/')[-1], pose_file_path.split('/')[-1]), fps=15, quality=5)
